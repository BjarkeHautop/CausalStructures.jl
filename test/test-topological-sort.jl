...
@testitem "topological_sort with isolated nodes" tags = [:unit] begin
    include("helpers.jl")
    cg = caugi(directed(:A, :B), node(:C); class = DAG)
    order = topological_sort(cg)
    @test length(order) == 3
    @test Set(order) == Set([:A, :B, :C])
    @test verify_topo_order(cg, order)
...
@testitem "topological_sort errors on ADMG" tags = [:unit] begin
    cg = caugi(directed(:L, :X), directed(:X, :Y), directed(:L, :Y); class = ADMG)
    @test_throws MethodError topological_sort(cg)
end

@testitem "topological_sort errors on PDAG" tags = [:unit] begin
    cg = caugi(directed(:A, :B), undirected(:B, :C), directed(:C, :D); class = PDAG)
    @test_throws MethodError topological_sort(cg)
end
...
@testitem "topological_sort NetworkX 1 test" tags = [:unit] begin
    include("helpers.jl")
    # A -> B, A -> C, B -> C
    cg = caugi(directed(:A, :B), directed(:A, :C), directed(:B, :C); class = DAG)
    order = topological_sort(cg)
    @test order == [:A, :B, :C]
    @test verify_topo_order(cg, order)
