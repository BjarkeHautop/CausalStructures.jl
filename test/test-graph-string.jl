@testitem "cgraph string: simple directed chain" tags = [:unit] begin
...
@testitem "cgraph string: fan-out with +" tags = [:unit] begin
    cg = cgraph("A --> B + C"; class = DAG)
    @test Set(nodes(cg)) == Set([:A, :B, :C])
    @test has_edge(cg, :A, :B)
...
@testitem "cgraph string: fan-in with +" tags = [:unit] begin
    cg = cgraph("A + B --> C"; class = DAG)
    @test has_edge(cg, :A, :C)
    @test has_edge(cg, :B, :C)
    @test length(cg.edges) == 2
...
@testitem "cgraph string: multiple statements separated by comma" tags = [:unit] begin
...
@testitem "cgraph string: reversed directed marker" tags = [:unit] begin
...
@testitem "cgraph string: undirected marker" tags = [:unit] begin
...
@testitem "cgraph string: bidirected marker" tags = [:unit] begin
...
@testitem "cgraph string: partial markers" tags = [:unit] begin
...
@testitem "cgraph string: isolated nodes" tags = [:unit] begin
    cg = cgraph("A --> B, C"; class = DAG)
    @test Set(nodes(cg)) == Set([:A, :B, :C])
...
@testitem "cgraph string: matches equivalent programmatic construction" tags = [:unit] begin
...
@testitem "cgraph string: newline-separated statements" tags = [:unit] begin
    cg = cgraph(
        """
        A --> B
        B --> C
...
@testitem "cgraph string: invalid graph for class throws" tags = [:unit] begin
    @test_throws Exception cgraph("A --> B --> A"; class = DAG)
end

@testitem "cgraph string: malformed syntax throws ArgumentError" tags = [:unit] begin
