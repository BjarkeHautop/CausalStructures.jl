...
@testitem "generate_graph: DAG with m=0 yields 0 edges and correct nodes" tags = [:unit] begin
    ...
    @test Set(nodes(g)) == Set([Symbol("V$i") for i = 1:n])
end
...
@testitem "simulate_data: errors on invalid samples" tags = [:unit] begin
    ...
    @test_throws ErrorException simulate_data(g; samples = 0)
    @test_throws ErrorException simulate_data(g; samples = -5)
end

@testitem "simulate_data: returns dict with correct keys" tags = [:unit] begin
    g = caugi(directed(:A, :B), directed(:B, :C); class = DAG)
    data = simulate_data(g; samples = 100, seed = 1)
    @test data isa Dict
...
@testitem "simulate_data: reproducible with seed" tags = [:unit] begin
    g = caugi(directed(:A, :B), directed(:B, :C), directed(:A, :C); class = DAG)
    data1 = simulate_data(g; samples = 100, seed = 42)
    data2 = simulate_data(g; samples = 100, seed = 42)
...
@testitem "simulate_data: different seeds produce different data" tags = [:unit] begin
    g = caugi(directed(:A, :B); class = DAG)
    data1 = simulate_data(g; samples = 100, seed = 1)
    data2 = simulate_data(g; samples = 100, seed = 2)
    @test data1[:A] != data2[:A]
end
...
@testitem "simulate_data: standardize=true yields mean≈0 and sd≈1" tags = [:unit] begin
    using Statistics
    g = caugi(directed(:A, :B), directed(:B, :C); class = DAG)
    data = simulate_data(g; samples = 1000, standardize = true, seed = 123)
    for (_, v) in data
...
@testitem "simulate_data: standardize=false does not standardize" tags = [:unit] begin
    using Statistics
    g = caugi(directed(:A, :B); class = DAG)
    data = simulate_data(g; samples = 1000, standardize = false, seed = 42)
...
@testitem "simulate_data: single node graph works" tags = [:unit] begin
    ...
    data = simulate_data(g; samples = 50, seed = 1)
    @test haskey(data, :A)
...
@testitem "simulate_data: graph with no edges (all exogenous)" tags = [:unit] begin
    ...
    data = simulate_data(g; samples = 100, standardize = false, seed = 1)
